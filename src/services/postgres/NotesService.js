...
class NotesService {
  ...
  async getNotes(owner) {
    const query = {
      text: 'SELECT notes.* FROM notes LEFT JOIN collaborations ON collaborations.note_id = notes_id WHERE notes.owner = $1 OR collaborations.user_id = $1 GROUP BY notes.id',
      values: [owner],
    };
    const result = await this._pool.query(query);
